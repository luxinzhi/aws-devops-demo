pipeline {
    ...
    stages {
        ...
        stage('Deploy Green') {
            steps {
                sh """
                    sed "s/IMAGE_TAG/${IMAGE_TAG}/g" k8s/green.yaml | kubectl apply -f -
        ...
        stage('Manual Approval') {
            steps {
                timeout(time: 5, unit: 'MINUTES') {
                    input(
                        message: 'Green deployment is healthy. Switch traffic?',
                        ok: 'Switch'
                    )
                }
            }
    ...
    post {
        ...
        failure {
            ...
            echo 'Deployment failed. Roll back to Blue.'
        ...
        aborted {
            ...
            echo 'Deployment aborted. Roll back to Blue.'
